...
public class DbScriptGeneratingManager {
    ...
    public void generateDbScriptV2(GenerateDbScriptRequestV2 request) throws IOException {
        String imageLocation = request.getImageLocation();
        String fileName = request.getFileName();
        String mangaName = request.getMangaName();
        String postTitle = mangaName+" Chapter ";
        String postName = postTitle.toLowerCase().replaceAll(" ", "-");
        Integer idTermManga = request.getIdTermManga();
        List<ImageCounterDto> imageCounterList = request.getImageCounterList();
        System.out.println("Count Of Chapter = "+imageCounterList.size());
        Integer startId = request.getStartIdScript();
        Integer seriesId = request.getSeriesId();
        ...
        String wpPostMetaScript= "";
        Date now = new Date();
        for (ImageCounterDto a : imageCounterList){
            now.setTime(now.getTime() + 1000);
            String finalDate = DateFormatUtil.getStandardFormat(now);
            String baseQueryB = ", 1, '"+finalDate+"', '"+finalDate+"', '";
            String endOfScriptB = "', '', '', '"+finalDate+"', '"+finalDate+"', '', 0, 'https://mangakulah.com/?p=";
            ...
            String finalContent = "";
            for (int c=1; c<=a.getImgCount(); c++){
                String content = imageLocation+ StringUtil.formatChapter3Digit(a.getChapterName())+"/"+StringUtil.formatNumber3Digit(c)+".jpg";
                finalContent = finalContent + baseContent + content + endContent;
            }
            wpPostScript = wpPostScript + baseQueryA+startId+baseQueryB+ finalContent+"', '"+postTitle+a.getChapterName()+
                    endOfScriptA+postName+(a.getChapterName().replaceAll("\\.", "-"))+endOfScriptB+startId+finalEndScript+System.lineSeparator();

            wpTermScript = wpTermScript + termBaseScript+startId+", "+idTermManga+", 0);"+System.lineSeparator()+System.lineSeparator();
            wpPostMetaScript = wpPostMetaScript + generatePostMetaScriptV2(startId, a.getChapterName(), seriesId)+System.lineSeparator();

            startId++;
            String wpPostScriptRevision =  baseQueryA+startId+baseQueryB+ finalContent+"', '"+postTitle+a.getChapterName()+
                    endOfScriptARev+endOfScriptBRev+startId+finalEndScriptRev+System.lineSeparator()+System.lineSeparator();
            wpPostScript = wpPostScript + wpPostScriptRevision;
...
//        }
    ...
    private String  generatePostMetaScriptV2(Integer postId, String chapter, Integer seriesId){
        String firstScript = "INSERT INTO `wp_postmeta` (`post_id`, `meta_key`, `meta_value`) VALUES\n" +
                "(";
        String secondScript = postId + ", '_edit_last', '1'),\n" +
                "(";
